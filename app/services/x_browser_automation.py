...
def get_stealth_browser(headless: bool = True, humanize: bool = True):
    """Launches CloakBrowser (CloakHQ) stealth Chromium with humanization or falls back gracefully."""
    try:
        return cloakbrowser.launch(headless=headless, humanize=humanize)
    except Exception as e:
        print(f"[CloakHQ Warning] Falling back to standard Playwright Chromium: {e}")
...
def get_browser_session_status() -> Dict[str, Any]:
    ...
    if not os.path.exists(SESSION_FILE):
        return {
            "has_session": False,
            "username": None,
            "engine": "CloakHQ Stealth Chromium",
            "updated_at": None,
    ...
    try:
        ...
        return {
            ...
            "details": "Browser session active (CloakHQ stealth mode enabled)" if (has_auth and has_ct0) else "Missing auth_token or ct0"
        }
    except Exception as e:
        return {
            "has_session": False,
...
def save_cookies_as_session(auth_token: str, ct0: str, username: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        "ok": True,
        "engine": "CloakHQ Stealth Chromium",
        "message": "X browser session successfully configured with CloakHQ stealth engine",
...
def send_x_dm_browser(
    ...
    """
    Sends a Direct Message on X (Twitter) using CloakHQ stealth browser automation
    ...
    browser = get_stealth_browser(headless=headless, humanize=True)
    ...
    try:
        ...
        # Handle Encrypted Chat PIN prompt if present
        if "pin" in page.url or page.query_selector('input[aria-label*="Digit 1"]'):
            pin = os.getenv("X_CHAT_PIN", "1435")
        ...
        input_selectors = [
            ...
            'div[contenteditable="true"][data-testid="dm-composer-textarea"]',
        ...
        return {
            ...
            "method": "cloakhq_stealth_browser_automation"
        }
...
def launch_interactive_login():
    """Launches headed CloakBrowser for one-time manual login to save cookies."""
    print("Launching headed CloakBrowser for X login. Please log in to your account...")
    browser = get_stealth_browser(headless=False, humanize=True)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "login":
        ...
        print("X CloakHQ Stealth Browser Automation Module. Status:", get_browser_session_status())
