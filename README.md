...
Designed specifically for engineers and founders who want clean, authentic, high-converting outreach with **Gmail SMTP**, **X (Twitter) Browser Automation**, **PDF resume attachments**, **disguised UTM parameter tracking**, and an **interactive template builder**.
...
- **Multi-Channel Dispatch**:
  ...
  - **X (Twitter) Direct Messages**: Free, cookie-based Playwright browser automation with human-like typing delays (no $100/mo API paywall).
...
| Layer | Technologies |
| :--- | :--- |
| **Backend** | Python 3.11+, FastAPI, SQLModel, SQLite, Pydantic, Playwright, UV |
| **Frontend** | Next.js 16 (App Router), React 19, Tailwind CSS v4, Lucide Icons, TypeScript |
| **Protocols** | SMTP (SSL:465), IMAP (SSL:993), Playwright Browser Automation |
...
│   │   └── twitter.py            # X Browser Automation & cookie session management
│   └── services/
...
│       └── x_browser_automation.py # Playwright X DM automation & session management
├── frontend/
│   ├── src/
│   │   └── app/
...
- Use the **Variable Placeholders** (`+{{firstName}}`, `+{{company}}`, `+{{custom_hook}}`, `+{{role}}`) and **Subject Presets** to dynamically customize messages.
- The repository comes pre-seeded with 5 genuine engineering & founder outreach templates.

### 3. Sending Personalized Outreach
- Click **"Message"** on any contact row in the table.
- Choose **Email (Gmail)** or **X Direct Message** (via browser automation).
- Toggle **"Attach Resume"** (checked by default).
- Toggle **"Disguise links with UTM parameters"** (checked by default).
- Click **"Send via Gmail"** or **"Send via X (Browser)"** to dispatch immediately.
